...
from lakeanalysis.nsidc import download_is2, read_atl03, read_atl06
from lakeanalysis.utils import dictobj, convert_time_to_string, read_melt_lake_h5

from earthdata_credentials.edcreds import getedcreds
uid, pwd, email = getedcreds()
if uid == '<your_user_id>':
    try:
        from ed_psa.edcreds import getedcreds
    except:
        print('To download data from NSIDC, you need to set up your Earthdata login credentials:')
        print('  In utils/earthdata_credentials/edcreds.py, add your credentials.')
        print('  ')
        print('  def getedcreds():')
        print("      return '<your_user_id>', '<your_secret_password>', '<your_email>'")
        print('  ')
        print('  Please remember not to push these to github or share them publicly elsewhere!')
...
def get_sentinel2_cloud_collection(lon, lat, date_time, days_buffer, buffer_m=2500, CLD_PRB_THRESH=40, BUFFER=100):
    ...
    datetime_requested = datetime.strptime(date_time, '%Y-%m-%dT%H:%M:%S')
...
def plotS2cloudfree(lon, lat, date_time, buffer_m=2500, max_cloud_prob=15, gamma_value=1.8, 
                    imagery_filename='imagery/my-satellite-image.tif', plot_filename='plots/sentinel2_cloudfree.jpg', ax=None):
    
    datetime_is2 = datetime.strptime(date_time, '%Y-%m-%dT%H:%M:%S')
    ...
    is2datetime = datetime.strptime(date_time, '%Y-%m-%dT%H:%M:%S')
...
def convert_time_to_string(dt):
    epoch = dt + datetime.timestamp(datetime(2018,1,1))
    return datetime.fromtimestamp(epoch).strftime("%Y-%m-%dT%H:%M:%S")
        
    
#####################################################################
def plotIS2S2(lon, lat, date, rgt, gtx, buffer_m=2500, max_cloud_prob=15, gamma_value=1.8, 
              imagery_filename='imagery/my-satellite-image.tif', plot_filename=None, title='ICESat-2 data',
              ylim=None, IS2dir='IS2data', return_data=False):
    ...
    gtxs, ancillary, photon_data = read_atl03(atl03file, geoid_h=False)
    df = photon_data[gtx]
    date_time = convert_time_to_string(df.dt.mean())
    ...
    myImage = plotS2cloudfree(lon=lon, lat=lat, date_time=date_time, buffer_m=buffer_m, 
        ...
        plot_filename='plots/plot.jpg', ax=ax1)
